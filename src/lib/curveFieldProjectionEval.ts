...
function sampleLutSigned(lut: Float32Array, t: number): number {
  ...
  const hdr = lut[leftIndex] + (lut[rightIndex] - lut[leftIndex]) * ratio;
  return hdrToSigned(hdr);
}
...
export function evaluateCompiledCurveFieldProjection(
  ...
  const [localX, localY] = worldToCurveFieldLocal(worldX, worldY, compiled.transform);
  ...
  const b = sampleLutSigned(compiled.channels.b, radialInteractionToT(localX, localY));
